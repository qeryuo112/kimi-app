...
## 十五、修复流式请求遗漏 `stream: true`（2026-08-27）

### 问题
此前 `chatWithAI` 虽然调用了 SSE 解析器并在日志中标记 `mode: "stream"`，但请求体实际没有发送 `stream: true`。上游因此返回普通非流式响应；长任务继续等待完整结果，月计划请求仍在 Cloudflare 120 秒处收到 524。短判题请求则可能被 SSE 解析器误当成空内容，随后落入本地兜底判分。

### 修复
- 请求体显式加入 `stream: true`。
- 400 流式不支持时，回退请求显式发送 `stream: false`。
- 流式函数检查 `Content-Type`：若上游返回普通 JSON，则解析完整 `choices[0].message.content`，内容为空或格式不符时明确报错，不再静默返回空字符串。
- 请求日志增加 `bodyStream` 字段，确认实际发送参数。

### 验证
生产判题请求已确认日志出现 `mode=stream`、`流式结束`；后续月计划请求应以 SSE 持续接收数据，避免 524。
