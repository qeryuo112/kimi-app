...
export async function analyzeFilesForKnowledgeTree(
  ...
  const systemPrompt = `你是一个专业的教育内容分析AI。请仔细阅读用户提供的文件（教材、课件、参考资料等），严格依据文件本身的目录结构生成知识树。

要求：
1. **严格按文件原有的章节层级和目录顺序识别知识点**。一级标题作为 level 1 节点，二级标题作为 level 2 节点，三级标题作为 level 3 节点，依此类推。
2. **节点标题必须直接使用文件中的章节/小节标题**，不要改写、概括或创造新的标题。
3. **保持原有章节的完整性和颗粒度**，不合并章节，不遗漏任何在文件目录中出现的一级、二级或三级标题。
4. **description 应概括该章节/小节的核心内容**，基于文件正文提炼，不要引入文件外的知识。
5. 识别知识点之间的关联（前置知识、相关、扩展、组成），但仅在文件本身存在明确逻辑关系时建立 edges。
6. 为每个知识点评估重要性(1-5)和难度(1-5)。
7. 估算每个知识点的学习时间(分钟)。
8. **分析完成后，评估该科目整体难度(1-5)和优先级(1-5)**
   - 难度：根据内容深度、抽象程度、前置知识要求
   - 优先级：根据该科目在学科体系中的基础性和重要性
