...
7. **分析文件内容所属的学科，以及每道题目考察的具体知识点，并返回在detectedSubject和detectedKnowledgePoint字段中**。注意：这两个字段必须是你从实际输入内容中明确识别出的真实学科和知识点名称，不要编造、泛化或自行分类。如果无法明确识别，请留空。
...
6. mixed模式下，必须混合至少2种不同题型
7. **分析题目所属学科和具体考察的知识点，并返回在detectedSubject和detectedKnowledgePoint字段中**。注意：这两个字段必须是你从实际输入内容中明确识别出的真实学科和知识点名称，不要编造、泛化或自行分类。如果无法明确识别，请留空。
8. **【重要】选项内容绝对不能包含答案提示或正确性标识。例如：不要写"（正确答案）"、"（正确）"、"（错误）"等任何暗示对错的文字；不要写"（黄酮...）"这种选项特有的解释性文字**${chemHint}
